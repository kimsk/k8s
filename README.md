## k8s

[How to distribute large workloads using Azure Batch](https://www.youtube.com/watch?v=wlJ3pUeFHBI)

[What is Azure Batch?](https://docs.microsoft.com/en-us/azure/batch/batch-technical-overview)

[Machine Learning and Containers at Scale: Azure Batch or Kubernetes?](https://www.linkedin.com/pulse/machine-learning-containers-scale-azure-batch-kubernetes-josh-lane/)

[Learn Kubernetes Basics](https://kubernetes.io/docs/tutorials/kubernetes-basics/)

[Kubernetes By Example](https://kubernetesbyexample.com/)

[Minikube vs. kind vs. k3s](https://brennerm.github.io/posts/minikube-vs-kind-vs-k3s.html#:~:text=Kind%20is%20another%20Kubernetes%20SIGs,very%20similar%20to%20minikube's%20approach.)

[Making Kubernetes Production Ready](https://applatix.com/making-kubernetes-production-ready/)

[Scalable Microservices with Kubernetes](https://www.udacity.com/course/scalable-microservices-with-kubernetes--ud615)

### Jobs

[Run Jobs](https://kubernetes.io/docs/tasks/job/)

[Jobs](https://kubernetes.io/docs/concepts/workloads/controllers/job/)

[Job patterns](https://kubernetes.io/docs/concepts/workloads/controllers/job/#job-patterns)

[Coarse Parallel Processing Using a Work Queue](https://kubernetes.io/docs/tasks/job/coarse-parallel-processing-work-queue/)

[Redis Best Practices](https://redislabs.com/redis-best-practices/introduction/)

[Kubernetes Cron/CronJobs in 10 minutes](https://www.youtube.com/watch?v=PUhqw0laR3A)

[Getting Started with Minikube on WSL2](https://matheja.me/2020/04/08/getting-started-with-minikube-on-wsl2.html)

[spring-batch-kubernetes](https://speakerdeck.com/benas/spring-batch-kubernetes?slide=22)

### local k8s using kind

[kind](https://kind.sigs.k8s.io/)

### local k8s dashboard

[Local Kubernetes with kind, Helm & Dashboard](https://medium.com/@munza/local-kubernetes-with-kind-helm-dashboard-41152e4b3b3d)

## Redis

[Production Checklist for Redis on Kubernetes](https://medium.com/swlh/production-checklist-for-redis-on-kubernetes-60173d5a5325)